...
const dashboardButton = document.getElementById("adaptiveDashboard");
...
function renderQuestion(question, answered = 0) {
  ...
  meta.innerHTML = `
    <div><strong>Progress</strong><span>${answered + 1} of ${QUIZ_LENGTH}</span></div>
...
async function startQuiz() {
  ...
  session = {
    ...
    step_title: data.step?.step_title || context.step_title,
...
form.addEventListener("submit", async (event) => {
  ...
  try {
    ...
    if (data.quiz_complete) {
      ...
      feedbackNode.innerHTML += `
        <p><strong>Result:</strong> ${result.correct_answers || 0}/${result.total_questions || QUIZ_LENGTH}
